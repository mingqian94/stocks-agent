...
|------|-----|
| 最小涨幅 | 3% |
| 最大涨幅 | 8%（原12%，2026-07-21调整，见下方说明） |
| 最小成交额 | 2亿 |
| 前一交易日确认 | 需要也是上涨（2026-07-21新增，见下方说明） |
| 持仓上限 | 2只 |
| 每只仓位 | 50%（2只满仓，不留现金缓冲——没有补仓策略不需要） |
| 止损线 | -5% |
...
**选股条件**：
1. 涨幅在 3%~8% 之间（排除涨幅过小或涨停、追高）
2. 前一交易日涨跌幅也是正的（排除"今天突然拉升但昨天在跌"的反弹/反转，只要正在延续的强势）
3. 成交额 > 2亿（排除流动性差的标的）
4. 选取动量最强的最多2只买入，补满空缺持仓位
...
>
> ⚠️ **2026-07-20 调整说明**：+20%止盈全历史几乎从未真正触发过（止损49次:止盈1次），拿真实买入记录回测过，2笔（300040/600722）在+20%规则下始终未落袋、换成+8%能提前锁定收益——改成+8%。
>
> ⚠️ **2026-07-21 调整说明**：拿CSI300+CSI500成分股(489只)、2026.06.02-07.21历史数据重建了每日候选池，回测了当前规则(3%-12%/止盈8%)——平均单笔-0.84%，胜率45%。分别单独试了"收紧涨幅带到3%-8%"和"加前一日确认过滤"，效果都比不改还差；但**两个一起加**，18笔交易胜率50%、平均单笔+2.67%，近似总资产从当前规则的-9.86%变成+24.41%。单独收紧涨幅带只是换了个方式追高（还是在追单日涨幅），单独加确认过滤在12%的宽带子里还是会选到接近涨停的标的；两个一起才是真正筛出"正在建立中、还没走完的强势"。所以这次两个改动一起上线，不单独只上一个。（回测样本489只非全市场股票、36个交易日，属于方向性验证，不是精确的收益预测）
