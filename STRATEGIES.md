...
| 板块/行业、公司主营业务没作为选股特征，可能是遗漏的重要维度 | 🔴 待验证 | 现在选股只看"涨幅+成交额+前日确认"，完全不看这只股票属于哪个行业、公司是做什么的。跟两个已有线索能对上：①7.20复盘查过实盘同时持仓的两只股票，9组组合里有1组是同行业(半导体)，说明现在选股确实会无意识地扎堆某个热门板块；②搜GitHub时见过"行业动量"这个研究方向（`lc-sysbs/Industries-Momentum`），选的是板块而不是个股。可能的方向：a) 选股时限制同时持仓不能是同一行业，分散风险；b) 干脆换成"行业/板块轮动"选股逻辑，而不是个股涨幅排名。这两个思路都还没回测验证，只是记录下来，不是现在就要做 |
| 东方财富的数据接口（`clist`选股接口、个股详情`/qt/stock/get`、akshare的`stock_zh_a_hist`等背后也是走东财源的函数）收盘后（15:00后）基本连不上，一直报`Connection aborted`/`RemoteDisconnected` | ⚠️ 环境限制 | 2026-07-27实测：收盘后测试`clist`、个股详情、`stock_zh_a_hist`全部失败，但同一时间新浪的实时行情接口(`hq.sinajs.cn`)访问正常——说明是东财这几个接口收盘后就下线/限制访问，不是被我们请求量打挂的（之前几次也怀疑过是自己测太猛触发限流，这次确认至少这一次是收盘导致的）。**下次要做涉及东财数据源的回测/研究，最好在交易时段内做**，收盘后再测这几个接口大概率白费功夫；新浪的实时行情接口不受此限制，但新浪没有现成的行业分类等数据，不能完全替代 |
